/* ARCRON KIT — integration cookbook for the Arcron keeper network.
   Live upkeep board decoded from keeper app 769891898 boxes on Algorand
   TestNet. Live algod fetch first, snapshot.json fallback. Read-only. */
(() => {
  ...
  /* ---- SHA-512/256 + Algorand address encoding (from corvid-agent/arcron-hub) ---- */
  ...
  // Upkeep box name: b"u" || itob(id) — 9 bytes, first byte 0x75. (TRAP 3)
  ...
  // Upkeep struct — big-endian, verified against keeper app 769891898:
  //   0  creator address (32 bytes)   32  target_app u64      42  interval_rounds u64
  //  50  next_execution_round u64     58  fee u64             66  balance u64
  //  74  times_executed u64           82  policy u64 (0 CATCH_UP = TRAP 6, 1 SKIP_AHEAD)
  ...
  // One-shot flap-in for static chrome (trap card numbers, section markers).
  function flapStatic() {
    document.querySelectorAll("[data-flap]").forEach((el) => {
      el.replaceChildren(flaps(el.dataset.flap, el.dataset.flapSize || ""));
    });
  }
  ...
  async function fetchLive() {
    ...
    const upkeeps = await Promise.all(names.map(async (name) => {
      ...
      // algonode needs the box name as ?name=b64:<base64>
  ...
  function policyBadge(p) {
    ...
    if (p === 0) return { text: "CATCH UP", cls: "catchup", title: "policy 0 · TRAP 6 · replays every missed interval · one due upkeep can demand many back-to-back executions" };
  ...
  function render(f) {
    ...
    f.upkeeps.forEach((u) => {
      ...
      fee.title = u.fee + " µALGO per execution (floor 4,000 — TRAP 9)";
